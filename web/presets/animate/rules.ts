import type { ControlSymbols, Rule } from '@unocss/core';
import type { Theme } from '@unocss/preset-wind4';
import { defineProperty, h } from '@unocss/preset-wind4/utils';

function defineOnce(prop: string, opts: any) {
  let defined = false;
  return () => {
    if (defined) {
      return;
...
function createSimpleRule(prop: string, def: any, transform: (val: string) => string | undefined, opts: {
  ...
  return function* ([, val]: RegExpMatchArray) {
...
function createBlurRule(prop: string, def: string) {
  return function* ([, val]: RegExpMatchArray) {
...
function createOpacityRule(prop: string, def: number) {
  return createSimpleRule(prop, def, val => h.cssvar.fraction.percent.px.rem(val), {
    initialValue: 1,
  });
}

function createScaleRule(prop: string, def: number) {
  return createSimpleRule(prop, def, val => h.cssvar.fraction.percent.px.rem(val), {
    initialValue: 1,
  });
}

function createRotateRule(prop: string, def: string) {
  return function* ([, val]: RegExpMatchArray) {
...
function createAnimationRule(prop: string, cssProp: string, resolver: (val: string, theme: Theme) => string | undefined, opts: any) {
  return function* ([, val]: RegExpMatchArray, { theme }: { theme: Theme }) {
    ...
    createAnimationRule(
      ...
      val =>
        ['initial', 'infinite'].includes(val)
          ? val
          : (h.bracket.cssvar.number(val) ?? val.replaceAll('-', ',')),
...
function createSlideRule(prefix: 'enter' | 'exit') {
  const propBase
  ...
  return function* (
    ...
    if (['start', 'end'].includes(dir)) {
      ...
      const vBracket = h.bracket(val);
      if (vBracket && (h.number(vBracket) || h.fraction(vBracket))) {
        return;
      }
